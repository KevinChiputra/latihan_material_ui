...
interface MovieTrenCardProps {
    movie: MovieDataType
}

const MovieTrendCard = ({movie}: MovieTrenCardProps) => {
    const {dispatch} = useContext(MovieContext)
    const handleToggleBokmark = (id: string) => {
        dispatch({type: "TOGLE BOOKMARK", id})
    }
    ...
    <Card key={movie.id} elevation={0} style={{backgroundColor: "transparent"}}>
        <CardContent style={{
            ...
            >
                ...
                onClick={() => handleToggleBokmark(movie.id)}
                >
